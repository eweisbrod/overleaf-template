...
% Variable Definitions table uses \supertabular (loaded in the preamble)
% rather than a regular tabular inside a [h!t!b] float. supertabular
% breaks naturally across page boundaries, so the appendix scales as
% the variable list grows. \tablefirsthead / \tablehead / \tabletail /
% \tablelasttail define the header and footer rows that repeat on each
% page the table spans. The caption is set via \topcaption (which
...
\topcaption{Variable Definitions \label{vars}}
...
%
% These are not used by the example regression; they exist purely to
% show how the supertabular environment breaks across pages once the
% variable list grows past a single page. Replace with your own
% variables in your paper, or delete this whole "Additional Controls"
% block if your project does not need a multi-page appendix.
% ---------------------------------------------------------------------
