...
This document is a \LaTeX\ template for creating an academic paper formatted for potential publication in Accounting or Finance. If you have received this document as a PDF, a live version including the \LaTeX\ source code is available at: \url{https://www.overleaf.com/read/ctmwnmdcypzh}. The companion GitHub coding templates are available at \url{https://github.com/eweisbrod/project-template-r} (R only) and \url{https://github.com/eweisbrod/project-template} (Python + R + Stata).
...
The example analysis uses an \textbf{earnings announcement event study}. We compute standardized unexpected earnings (SUE) as the seasonal change in quarterly income scaled by prior-quarter market value, building on the classical SUE literature \citep[e.g.,][]{ball1968,bernard1989post}. We test whether the three-day buy-and-hold abnormal return around the announcement is stronger when the direction of the sales change confirms the earnings change. \citet{ertimur2003differential} also examine how non-earnings line items affect the market response to earnings.\footnote{Footnotes are useful for tangential points or extended citations. They appear at the bottom of the page automatically.}

In contrast, an earlier paper by \citet{ball1968} began the literature on price reactions to earnings news. The first time we cite a paper, the full author list appears; subsequent citations of the same paper use ``et al.'' (e.g., \citet{ball1968}). \citepos{bernard1989post} possessive form is also useful when you want to write ``Bernard and Thomas's [1989] finding that\ldots''
...
If we wanted to define an open-ended ``research question'' rather than a directional hypothesis, we could use the parallel \texttt{researchq} environment instead:

\begin{researchq}[RQ\ref{researchq:size}] \label{researchq:size}
Does the market reaction to earnings surprises vary with firm size?
\end{researchq}

Both environments use \texttt{ntheorem} machinery defined in the preamble. The auto-numbering means you can reorder, add, or delete them without manually renumbering --- references such as H\ref{hyp:samesign} and RQ\ref{researchq:size} update automatically.

\subsection{Subsection Example}\label{sec:subsection-example}

This is a subsection. Subsections are useful for organizing the background and motivation into thematic blocks (e.g., one subsection per related literature stream). The subsection numbering format is set in the preamble via \texttt{titlesec}.

\subsubsection{Sub-subsection Example}\label{sec:subsubsection-example}

This is a sub-subsection. They are rarely needed in published papers but can be helpful when drafting.
...
Our sample begins with Compustat quarterly fundamentals (fundq) for fiscal years 1970--2024 with non-missing earnings announcement dates. We merge with the CRSP/Compustat Linking Table (CCM) to obtain CRSP security identifiers, and with CRSP stocknames for SIC codes. Following standard practice in the accounting literature \citep[e.g.,][]{ball1968,bernard1989post}, we exclude financial firms (SIC 60--69) and utilities (SIC 49). Table~\ref{tab:sample-selection} details the sample selection procedure.
