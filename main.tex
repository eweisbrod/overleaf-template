...
% the content. Vertical (stacked) panelling lets each panel use ~90% of
% textwidth, which is much easier to read for plots with many tick
% marks, dense legends, or text-heavy axes. Horizontal side-by-side
% (Figure 2) is preferable when the visual comparison BETWEEN the two
% panels is the main message and the plots have minimal axis text.
\begin{figure}[h!t!b]
\captionsetup{font=normal}
...
\centering
\subfloat[SameSign Frequency by Size Quintile Over Time]{%
  \includegraphics[width=0.9\textwidth]{figures/size_year.pdf}}\\[1ex]
\subfloat[Year-by-Year ERC with Confidence Bands]{%
  \includegraphics[width=0.9\textwidth]{figures/erc_year.pdf}}
\par\medskip
\footnotesize
\begin{minipage}{\textwidth}
\begin{singlespace}
